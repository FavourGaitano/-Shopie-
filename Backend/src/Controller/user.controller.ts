...
export const createUser = async (req: Request, res: Response) => {
  try {
    let id = v4();
    console.log("User:", id);
    const { name, email, phone_no,  password}: User = req.body;
...
export const updateUser = async (req: Request, res: Response) => {
    try {
      ...
      let user = (await (Connection.execute("updateUser", { user_id:id, name, email, phone_no, password:hashed_pwd}))).recordset
    ...
    export const deleteUser = async (req: Request, res: Response) => {
        try {
          const id = req.params.user_id;
          console.log("User ID:", id);
          let user = (await Connection.execute("deleteUser", { user_id: id })).recordset
