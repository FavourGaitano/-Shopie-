...
export const createOrder = async (req: Request, res: Response) => {
  try {
    ...
    // checking  if order already  exists in the database by its cart id
    const result = (await pool
      .request()
      .input('cart_id', mssql.VarChar, cart_id)
      .execute("CheckOrderExists")).rowsAffected;

      console.log("Your result",result[0]);

      if(result[0]>= 1){
        return res.status(503).json({message:"This order already exists"});
              ...
              const createOrder = (
                ...
                .execute('createOrder')).rowsAffected;
...
export const getOrderByUserId = async (req: Request, res: Response) => {
  try {
      ...
      let orderDetails = (await Connection.execute("getOrderByUserId", { user_id: user_id })).recordset;
      ...
      return res.status(400).json({ message: "There was an issue retrieving the orders." });
  }
};
    ...
    export const cancelOrder = async (req: Request, res: Response) => {
        try {
          ...
          let order = (await Connection.execute("cancelOrder", { order_id: id })).recordset
    ...
    export const changeStatus = async (req: Request, res: Response) => {
      try {
        const id = req.params.order_id;
        console.log("Order ID:", id);
        let order = (await Connection.execute("changeStatus", { order_id: id })).recordset
